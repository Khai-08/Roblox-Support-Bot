...
class GuildPaginator(View):
    def __init__(self, bot, ctx, guild_names, per_page=10):
        super().__init__()
        self.bot = bot
        self.ctx = ctx
...
def setup(bot):
    @bot.command(name='analytics', aliases=['an'])
    @bot.cmd_logger
    async def analytics(ctx):
        if ctx.author.id != bot.owner:
            return await bot.error_embed(ctx, "You must be the bot owner to use this command. Only the owner can execute this command.")
        ...
        try:
            ...
            if len(guild_names) > 10:
                view = GuildPaginator(bot, ctx, guild_names)
